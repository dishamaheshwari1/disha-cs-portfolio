  ...
  return (
    <div className="px-6 md:px-10 max-w-6xl mx-auto pb-16">
      <Section className="text-center mb-16">
        ...
        <FirstLetterLarge text="Research" className="text-creamy text-5xl md:text-7xl" />
        <p className="mt-8 max-w-xl mx-auto text-creamy/80">
          Essays, working notes and field logs. Updated when there's something worth saying — not before.
        </p>
      </Section>
            ...
            >
              <div>
                <div className="flex items-center justify-between mb-4">
                  <span className="text-[10px] uppercase tracking-[0.3em] text-carnelian">{e.tag}</span>
                  <ArrowUpRight
                    className="w-4 h-4 text-minimalist group-hover:text-creamy transition-colors"
                    strokeWidth={1.5}
                  />
                </div>
                <h3 className="text-creamy text-xl uppercase tracking-wide leading-tight">{e.title}</h3>
                <p className="mt-3 text-sm text-creamy/75">{e.body}</p>
              </div>
              <p className="mt-6 text-[10px] uppercase tracking-[0.3em] text-minimalist">{e.date}</p>
            </Link>
          ))}
        </div>
      </Section>
