...
function ProgressBar({ value, color }: { value: number; color: "navy" | "cream" }) {
  ...
  const fill = color === "navy" ? "var(--color-carnelian)" : "var(--color-lace)";
  return (
    <div className="flex gap-[3px] h-5">
      {Array.from({ length: segments }).map((_, i) => (
        <div
          ...
          style={{ backgroundColor: i < filled ? fill : "transparent" }}
        />
      ))}
    </div>
  );
        ...
        <li key={it.text} className="flex items-start gap-3">
          <span
            ...
            className={`mt-1 w-5 h-5 shrink-0 border flex items-center justify-center ${
              it.done ? "bg-carnelian border-carnelian" : "border-lace/50 bg-transparent"
            }`}
          >
            {it.done && <Check className="w-3.5 h-3.5 text-creamy" strokeWidth={3} />}
          </span>
          <span className={`text-sm md:text-base select-none ${it.done ? "text-creamy/60" : "text-creamy"}`}>
            {it.text}
          </span>
        </li>
      ))}
